...
u64 MimeIdCheck(SFXOBJ *pObj, s32 StartNum, s32 EndNum) {
    ...
    pSfz0 = Fadr(pObj->MimeAdrs, StartNum);
    pSfz1 = Fadr(pObj->MimeAdrs, EndNum);
...
void SetSyncMime(SFXOBJ *pObj) {
    ...
    for (ii = 0; ii < MimeNum; ii++) {
        MimeSet(pObj, pMime[0], pMime[1], pMime[2] / 255.0f);
        pMime += 3;
    }
}
...
void MimeSet(SFXOBJ *pObj, s32 StartNum, s32 EndNum, f32 Weight) {
    ...
    IdFlag = 0;
    for (i = 0; i < pSfz0->parts_num; i++, pSfzParts0++) {
        id = pSfzParts0->parts_id;
        IdFlag |= 1 << id;
    }
        ...
        while (pObjTmp != NULL) {
            if ((pObj->Condition & pObjTmp->Flag) != 0) {
                if (pObjTmp->MimeAdrs == NULL)
                    return;
                pSfz0 = Fadr(pObjTmp->MimeAdrs, StartNum);
                pSfz1 = Fadr(pObjTmp->MimeAdrs, EndNum);
                ...
                for (i = 0; i < pSfz0->parts_num; i++, pSfzParts0++, pSfzParts1++) {
                    ...
                    pObjTmp->pParts[id].vert_adrs_mime0 = (int *)((uintptr_t)pSfz0 + pSfzParts0->vert_adrs);
                    pObjTmp->pParts[id].norm_adrs_mime0 = (int *)((uintptr_t)pSfz0 + pSfzParts0->norm_adrs);
                    pObjTmp->pParts[id].MimeWeight = Weight;
                    pObjTmp->pParts[id].MimeStart = StartNum;
                    pObjTmp->pParts[id].MimeEnd = EndNum;

                    id = pSfzParts1->parts_id;
                    pObjTmp->pParts[id].vert_adrs_mime1 = (int *)((uintptr_t)pSfz1 + pSfzParts1->vert_adrs);
                    pObjTmp->pParts[id].norm_adrs_mime1 = (int *)((uintptr_t)pSfz1 + pSfzParts1->norm_adrs);
                }
            }
